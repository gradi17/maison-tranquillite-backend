...
app.post('/api/checkout', async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      // Klarna, Scalapay...). Si vous activez un nouveau moyen de
      // paiement plus tard, il apparaîtra ici sans retoucher ce fichier.
      line_items,
...
// Envoie l'email de livraison avec les liens de téléchargement, via Resend.
async function sendDownloadEmail(email, lineItems) {
  ...
  const itemsListHtml = lineItems.map((li) =>
    `<li>${li.description} — <a href="https://votredomaine.fr/downloads/${encodeURIComponent(li.description)}">Télécharger</a></li>`
  ).join('');
  // Remplacez ce lien par vos vrais liens de téléchargement (idéalement
  // des liens signés/expirants générés depuis votre stockage de fichiers).
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      ...
      html: `
        ...
        <p>Voici vos liens de téléchargement :</p>
        <ul>${itemsListHtml}</ul>
