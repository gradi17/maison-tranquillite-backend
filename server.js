...
// IMPORTANT — stockage : toutes les données (catalogue, PDF, images)
// vivent sous DATA_DIR. En local, ça pointe simplement dans ce dossier.
// Sur Render, réglez la variable d'environnement DATA_DIR sur le chemin
// de montage de votre disque persistant (ex: /var/data) pour que rien
// ne soit jamais perdu lors d'un futur déploiement de code.
...
const EVENTS_FILE = path.join(DATA_DIR, 'events.json');   // visites & clics
const ORDERS_FILE = path.join(DATA_DIR, 'orders.json');   // commandes payées
const TAGS_FILE = path.join(DATA_DIR, 'tags.json');       // tags suggérés
const THEME_FILE = path.join(DATA_DIR, 'theme.json');     // couleurs du site
const FILES_DIR = path.join(DATA_DIR, 'files');       // PDF — jamais servis publiquement
const IMAGES_DIR = path.join(DATA_DIR, 'images');     // couvertures/aperçus — servis publiquement
...
app.get('/robots.txt', (req, res) => {
  res.type('text/plain').send(
    'User-agent: *\nDisallow: /admin\nDisallow: /api/admin\n\nSitemap: https://pornxhentai.com/sitemap.xml'
  );
});
...
app.get('/sitemap.xml', (req, res) => {
  const books = loadBooks();
  const urls = [
    'https://pornxhentai.com/',
    ...books.map(b => `https://pornxhentai.com/livre/${b.id}-${slugify(b.title)}`),
  ];
...
app.get('/livre/:slug', (req, res) => {
  ...
  const description = book.description || `${book.title}, par ${book.author || 'auteur inconnu'}. ${book.category}. Tags : ${book.tags.join(', ')}.`;
  const canonical = `https://pornxhentai.com/livre/${book.id}-${slugify(book.title)}`;

  res.send(`<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="UTF-8">
<title>${book.title} — PornXHentai</title>
...
<body>
  <div class="wrap">
    ...
    ${book.description ? `<p style="text-align:left;font-size:14px;line-height:1.6;">${book.description}</p>` : ''}
...
app.post('/api/admin/books', requireAdmin, bookUpload, (req, res) => {
  try {
    const books = loadBooks();
    const { title, author, category, price, tags, mature, color, description } = req.body;
    ...
    res.status(500).json({ error: 'Erreur serveur lors de l\'ajout du livre: ' + err.message });
  }
});
...
app.put('/api/admin/books/:id', requireAdmin, bookUpload, (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erreur serveur lors de la modification du livre: ' + err.message });
  }
});
...
async function sendDownloadEmail(email, itemIds) {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({
      from: 'PornXHentai <commandes@pornxhentai.com>',
      to: email,
      ...
      html: `
        ...
        <p>Bonne lecture,<br>PornXHentai</p>
      `,
      attachments,
    }),
  });
